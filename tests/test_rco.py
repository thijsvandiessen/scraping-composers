...
from composer_ingest.scraper.sources.rco.artists import (
    _Credit,
    collect_credits,
    credit_record,
    iter_conductor_records,
)
from composer_ingest.scraper.sources.rco.performances import _performances
...
def test_iter_conductor_records_skips_non_person_group_blocks() -> None:
    page: dict = {"content": [{"type": "intro", "text": "<p>...</p>"}]}
...
def test_performances_all_intervals_returns_empty() -> None:
    concert = {**CONCERT, "program": [{"nameEn": "-- interval --", "relatedCredit": "-- interval", "durationMinutes": 20}]}
