...
1. one ``work`` record per work performed (``performances``), linking it to its
   composer/conductor/soloists, orchestra, period and date as claims;
...
def fetch_records(max_pages: int | None = None) -> Iterator[SourceRecord]:
    """Yield every work-performance in the archive (one ``work`` record each),
