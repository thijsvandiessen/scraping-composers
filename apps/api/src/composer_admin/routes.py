...
from composer_ingest.etl.gold import DEFAULT_GOLD_DB_PATH, promote, read_gold_manifest
from composer_ingest.etl.ingestion import create_run, execute_run
from composer_ingest.etl.models import IngestRun, utcnow
from composer_ingest.scraper.bucket import DEFAULT_BUCKET_PATH, LOADABLE_STATUSES, LocalBucket, Snapshot
from composer_ingest.scraper.scraper import Scraper, iter_from_bucket, new_snapshot_id
from composer_ingest.scraper.sources import REGISTRY, SourceAdapter, is_due
...
def _promote_in_background() -> None:
...
def _gold_status() -> GoldStatus:
    manifest = read_gold_manifest(DEFAULT_GOLD_DB_PATH)
    return GoldStatus(
        exists=Path(DEFAULT_GOLD_DB_PATH).exists(),
