"""Claim collection for the gold build: deduped, and the rule 3 walk."""
...
def collect_person_claims(build: GoldBuild) -> set[uuid.UUID]:
    """Claims of kept persons, deduped. Returns rule 3's seed — the non-kept
    entities those claims reference by at least ``min_referrers`` distinct kept
    persons (all of them when the rule is off)."""
    # Ingest already collapses identical assertions per source, so this rarely
    # fires; it stays as a cheap guard against a source repeating itself.
    seen_claims: set[tuple[uuid.UUID, str, uuid.UUID | None, str | None, int]] = set()
    referrers: dict[uuid.UUID, set[uuid.UUID]] = {}
    for chunk in chunked(sorted(build.kept_persons, key=str)):
        for c in build.silver.execute(
            select(Claim).where(Claim.subject_id.in_(chunk)).order_by(Claim.id)
        ).scalars():
            key = (c.subject_id, c.predicate, c.object_id, c.value, c.source_id)
            if key in seen_claims:
                continue
            seen_claims.add(key)
            obj = c.object_id
            if obj is not None and obj not in build.kept_persons:
                referrers.setdefault(obj, set()).add(c.subject_id)
...
def walk_referenced(build: GoldBuild, referenced: set[uuid.UUID]) -> None:
    ...
    frontier = {r for r in referenced if r not in build.kept_persons and r not in build.unevidenced_ensembles}
    while frontier:
    ...
    if not build.config.prune_unreferenced:
        build.kept_other |= build.all_other - build.kept_persons - build.unevidenced_ensembles
    ...
    build.kept_other |= build.kept_ensembles - build.kept_persons
...
def drop_pruned_object_claims(build: GoldBuild) -> None:
    ...
    kept = build.kept_persons | build.kept_other
    build.claim_rows = [
