...
from composer_ingest.scraper.sources.wikidata.parse import _format_time, _records_from_rows
from composer_ingest.scraper.sources.wikidata.query import _fetch_metrics, _fetch_page, _run_query
...
def test_truncated_body_is_retried_via_uncached_post(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("composer_ingest.scraper.sources.wikidata.time.sleep", lambda _: None)
...
def test_run_query_raises_after_all_retries_exhausted(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("composer_ingest.scraper.sources.wikidata.query.time.sleep", lambda _: None)
...
def test_run_query_honors_retry_after_header(monkeypatch: pytest.MonkeyPatch) -> None:
    sleeps: list[float] = []
    monkeypatch.setattr("composer_ingest.scraper.sources.wikidata.query.time.sleep", sleeps.append)
    attempts: list[int] = []
