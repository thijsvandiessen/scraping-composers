...
import httpx
import pytest
from composer_ingest.scraper.sources.rco.fetch import (
    BASE_URL,
    _get_text,
    _make_client,
    page_slugs,
...
def _patch_client(monkeypatch: pytest.MonkeyPatch, handler: Any) -> None:
    ...
    monkeypatch.setattr("composer_ingest.scraper.sources.rco.fetch.httpx.Client", _MockedClient)
...
def test_get_text_retries_on_server_error(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("composer_ingest.scraper.sources.rco.fetch.time.sleep", lambda _: None)
    attempts: list[int] = []
