...
def test_run_query_raises_after_all_retries_exhausted(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("composer_scrapers.wikidata.query.time.sleep", lambda _: None)
...
def test_run_query_honors_retry_after_header(monkeypatch: pytest.MonkeyPatch) -> None:
    sleeps: list[float] = []
    monkeypatch.setattr("composer_scrapers.wikidata.query.time.sleep", sleeps.append)
    attempts: list[int] = []
