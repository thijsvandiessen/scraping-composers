...
import pandas as pd
import pytest
from composer_scrapers._pdf import PdfSourceAdapter, _fetch_pdf_bytes
from composer_scrapers.classicalcomposersposter import ClassicalComposersPosterAdapter
from composer_scrapers.classicalcomposersposter.parse import _infer_date_columns, _parse_rows

_DC_PATH = "composer_scrapers.classicalcomposersposter.parse.DocumentConverter"
...
def test_fetch_pdf_bytes_retries_on_http_error(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("composer_scrapers._pdf.time.sleep", lambda _: None)
    attempts: list[int] = []
...
def test_fetch_yields_entity_documents(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "composer_scrapers.classicalcomposersposter.ClassicalComposersPosterAdapter._download_pdf",
        lambda self: b"fake",
    )
    monkeypatch.setattr(
        "composer_scrapers.classicalcomposersposter._parse_rows",
...
def test_parse_rows_logs_warning_on_markdown_fallback(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with patch(_DC_PATH, return_value=mock_converter):
        with caplog.at_level(logging.WARNING, logger="composer_scrapers.classicalcomposersposter.parse"):
            rows = _parse_rows(b"fake")
